...
$(document).data("fieldList", {
   Halo: [5,11,21,22,23,18,19,20,10,13,17,12,16,14,15,6],
   Field: [4,5,6,9,12,8,7,11,10,14,13,0]
});
...
jQuery(document).ready(function() {
    ...
    $("#checkAllProperties").live("click", function(){        
            $("#propertiesRangeQueryTable :input").attr("checked",$(this).attr("checked"));        
    });
    ...
    $(".startSimu").click(function() {

        var params = JSON.parse($(this).attr('data'));

        $(".startSimu").css({"font-weight": "normal", "text-decoration": "none"});
        ...
        $(document).data("userSelection").physicId = 7;
        $(document).data("userSelection").protocolId = 2;
        $(document).data("userSelection").boxLength = params.len;
        $(document).data("userSelection").resolution = params.res;
        $(document).data("userSelection").cosmology = params.cosmoid;
    ...
    $("#1FormButton").click(function() {
        ...
        console.log($(document).data("userSelection"));
    ...
    function updateObjectTypeForSimu() {
        ...
        if ($("#matchingSimulations").val()) {
            ...
            function(objectTypeList) {
                ...
                $('#selectTypeObject').show();
                $('#objectList').html('');
                jQuery.each(objectTypeList, function(index, objectType) {
                    $('#objectList').append('<input type="radio" value="' + objectType.name + '" id="' + objectType.name + '" name="objectType" class="objectTypeSelector"><label for="' + objectType.name + '"> ' + objectType.name + '</label> : <em>' + objectType.description + '</em><div id="type_' + objectType.name + '"></div><br/><br/>');
                    ...
                    function(postprocessorList) {
                        jQuery.each(postprocessorList, function(index, postprocessor) {
                            var parameters = getParametersForProtocol(postprocessor.id);
                            
                            jQuery.each(parameters, function(index, protocol) {
                                $.ajax({
                                    url: 'ajax/availableParameterSettingForParameterPostprocessorParent',
                                    dataType: 'json',
                                    data: {
                                        inputParameterId: protocol.id,
                                        protocolId: postprocessor.id,
                                            ...
                                            function(paramValuesList) {
                                                jQuery.each(paramValuesList,
                                                        function(index, pvalue) {                                                            
                                                            $('#type_' + objectType.name).html(protocol.description+": &nbsp; " + pvalue.numericvalue_value);
                                                            ParamsByType[objectType.name] = {};
                                                            ParamsByType[objectType.name]['inputParamSetting'] = pvalue.numericvalue_value;
                                                            ParamsByType[objectType.name]['inputParamId'] = protocol.id;
                                                            ParamsByType[objectType.name]['postprocessorId'] = postprocessor.id;                                                            
                                                        }                                                        
                                                );                                                    
                                            }                                            
                                });
    ...
    $('.objectTypeSelector').live('click', function() {        
        var objectType = $(this).attr('id');
        
        $(document).data("userSelection").paramSetting = ParamsByType[objectType]['inputParamSetting'];
        $(document).data("userSelection").parameterId = ParamsByType[objectType]['inputParamId'];
        $(document).data("userSelection").postProcessorId = ParamsByType[objectType]['postprocessorId']; 
        $(document).data("userSelection").postprocParamsSetting = {};
        $(document).data("userSelection").postprocParamsSetting[ParamsByType[objectType]['inputParamId']] = ParamsByType[objectType]['inputParamSetting'];
        $(document).data("userSelection").objectType = objectType;
    ...
    var mandatoryOutputProperties = [];
        mandatoryOutputProperties['Halo'] = ["x", "y", "z", "npart", "mass"];
        mandatoryOutputProperties['Field'] = ["x", "y", "z", "npart"];
        
    //Put here properties that are "important"
    var importantOutputProperties = [];
        importantOutputProperties['Halo'] = ["x", "y", "z", "npart", "mass","vx","vy","vz","rmax","vmax","idp"];
        importantOutputProperties['Field'] = ["x", "y", "z", "npart", "idp"];    
    ...
    function updatePropertiesStatsForObjecttype() {
        ...
        function(propertyStatList) {
            ...
            tabPropHeaderStr += '<th>View in result <input type="checkbox" id="checkAllProperties"></th>\n';
            $("#propertiesRangeQueryTable").append(tabPropHeaderStr + '</tr>' + '\n');

            // print values
            //jQuery.each(propertyStatList, function(index, propertyStat) {
            jQuery.each($(document).data("fieldList")[$(document).data("userSelection").objectType], function(index, propertyStatId) {
                var mandatory = false;
                var important = false;
                propertyStat = propertyStatList[propertyStatId];
                if (jQuery.inArray(propertyStat.colName, mandatoryOutputProperties[$(document).data("userSelection").objectType]) >= 0) {
                    mandatory = true;
                }                
                if (jQuery.inArray(propertyStat.colName, importantOutputProperties[$(document).data("userSelection").objectType]) >= 0) {
                    important = true;
                }                                
                var mandatoryClass = mandatory ? ' mandatory' : '';
                var importantClass = important ? '' : ' secondary';
                var unit = (propertyStat.unit.search(/dimensionless/i)) ? "<i><small>(" + propertyStat.unit + ")</small></i>" : "";
                var addBorderClass = "";
                
                // add separator
                if(propertyStatId == 12 && $(document).data("userSelection").objectType == 'Halo') {
                    addBorderClass = ' addBorder';
                }   
                
                var propRowStr = '<tr class="'+mandatoryClass+importantClass+addBorderClass+addBorderClass+'"><td><div name="' + propertyStat.propName + '">' + propertyStat.propName + ' ' + unit + '</div></td>\n';
                ...
                for (j = 0; j < statNameList.length; j++) {
                    ...
                    var myRe = new RegExp(statNameList[j]+" "+propertyStat.propName,"i");// + "\\s+[^\\s]+$", "i");
                    var found = false;
                    jQuery.each(propertyStat.statList, function(indexStat, pStat) { // COULD BE IMPROVED USING "IN_ARRAY" FROM JQUERY
                        ...
                        if(pStat.name.toLowerCase() == (statNameList[j]+" "+propertyStat.propName).toLowerCase()) {
                            found = true;
                            //$("#propertiesRangeQueryTable > tbody").append('<td><div name="'+pStat.name+'">'+beautifulNumber(pStat.numericvalue_value,2)+'</div></td>\n');
                            propRowStr += '<td><div id="' + pStat.name + '">' + (propertyStat.propName == "idp" ? pStat.numericvalue_value : beautifulNumber(pStat.numericvalue_value, 2)) + '</div></td>\n';
    ...
    $('button[name=searchAllObject]').click(function() {
        $('#propertiesRangeQueryTable :input[type="checkbox"]').attr("checked",true);
        $('#propertiesRangeQueryTable :input[type="text"]').attr("value","");
        submitQueryProducts();
    });
    ...
    function submitQueryProducts(resultPageToDisplay) {
        ...
        $('#matchingProductsDiv').show();

        if (typeof(resultPageToDisplay) == 'undefined') {
            resultPageToDisplay = 1;
        }

        wantedPropertyNameList = mandatoryOutputProperties[$(document).data("userSelection").objectType];//see above
        ...
        $("#propertiesRangeQueryDiv input:checkbox:checked").each(function(idx) {
            if($(this).attr("id") != "checkAllProperties") {
        ...
        if (wantedPropertyNameList.length > 0) {
            ...
            function(res) {
                ...
                if (res.resultCount == 0) {
                    ...
                    $("#productsNumber").html("<h3>There are " + formatNumber(res.resultCount) + " matching objects</h3><a href='#download'>Directly go to download links</a><br/>");//(sorted by "+orderCriterion+")");

                    $("#productDownloadLink").html('<a name="download"></a><div class="download_bloc" id="productDownloadLinkBloc"></div><div class="download_bloc" id="productDownloadExtractBloc"></div>');

                    // Download bloc
                    $("#productDownloadLink #productDownloadLinkBloc").html('<h4>Download properties of all matching objects</h4>');
                    $("#productDownloadLink #productDownloadLinkBloc").append("<br/><em>Be careful, only properties selected as \"View in result\" in the above table will be exported</em><br/><br/>\n");
                    if (res.productListTextURL != "") {
                        $("#productDownloadLink #productDownloadLinkBloc").append('<br/><button name="downloadText" style="cursor:pointer;">Download as Text file</button>');
                        $("#productDownloadLink button[name='downloadText']").click(function() {
                            window.open(res.productListTextURL);
                        });
                    }

                    if (res.productListVotableURL != "") {
                        $("#productDownloadLink #productDownloadLinkBloc").append('<br/><br/><button name="downloadVotable" style="cursor:pointer;">Download as VOTable XML file</button>');
                        $("#productDownloadLink #productDownloadLinkBloc").append("<br/><em>to be used with software such as Topcat : <a href='http://www.star.bris.ac.uk/~mbt/topcat/' target='_blank'>http://www.star.bris.ac.uk/~mbt/topcat/</a></em><br/><br/>\n");
                        $("#productDownloadLink button[name='downloadVotable']").click(function() {
                            window.open(res.productListVotableURL);
                        });
                    }


                    // Extract bloc
                    $("#productDownloadLink #productDownloadExtractBloc").html('<h4>Select and Extract particles as binay files</h4>');
                    $("#productDownloadLink #productDownloadExtractBloc").append("<br/><em>To extract particles as binary files, you first have to select the objects in the list below then click on the Extract button.\n\
                                                    <br/>You will then have to enter your e-mail address and will automatically receive a link to download your files frome this adress.</em><br/>");
                    $("#productDownloadLink #productDownloadExtractBloc").append('<br/><button name="launchExtraction" style="cursor:pointer;" >Extract selected objects</a>');

                    $("#productDownloadLink button[name='launchExtraction']").click(function() {
                        rest_ExtractObjectsCheckmail();
                    });//"javascript:rest_ExtractObjectsCheckmail();return false;"
                ...
                $(document).scrollTop( $(':button[name="searchMatchingObject"]').offset().top );  
            },
                    "json");
        ...
        if ($("#matchingProducts tbody tr").length == 0) {
            ...
            for (var i = 0; i < globalProductList.length; i++) {
                ...
                jQuery.each(wantedPropertyNameList, function(ind, prop) {
                    strToAppend = strToAppend.concat('<td>' + (prop == "idp" ? product[prop] : beautifulNumber(product[prop], 2)) + '</td>');
                });
        ...
        if ($(that.domTableId + " tbody tr").length == 0) {
            ...
            for (var i = 0; i < effectiveDsSize; i++) {
                ...
                jQuery.each(that.propertyNameToDisplayList, function(ind, prop) {
                    strToAppend = strToAppend.concat('<td>' + (prop == "idp" ? object[prop] : beautifulNumber(object[prop], 2)) + '</td>');
